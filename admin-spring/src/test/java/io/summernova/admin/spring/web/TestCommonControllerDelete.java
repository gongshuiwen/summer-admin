...
class TestCommonControllerDelete extends CommonControllerTestBase {
    ...
    void testAuthorized() throws Exception {
        ...
        deleteIds.forEach(id -> Assertions.assertNotNull(mockService.getBaseMapper().selectById(id)));

        ResultActions resultActions = doDelete(deleteIds);

        checkResultActionsSuccess(resultActions, true);
        deleteIds.forEach(deleteId -> Assertions.assertNull(mockService.getBaseMapper().selectById(deleteId)));
    }
